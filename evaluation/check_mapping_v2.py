...
def get_all_xinyun_categories(char):
    """Return all possible Xinyun categories for a character (all readings)."""
    try:
    ...
    categories = set()
    for init, final in zip(all_initials, all_finals):
        cat = map_final_to_xinyun(init, final)
        if cat != 'none':
            categories.add(cat)
    return categories
...
for expected_category in xinyun:
    for tone in ['平', '仄']:
        for char in xinyun[expected_category].get(tone, []):
            ...
            if not all_cats:
                still_unmatched.append(char)
            elif expected_category in all_cats:
                correct += 1
            else:
                mismatches[(expected_category, tuple(sorted(all_cats)))].append(char)

out = 'C:/code/Constrained_decoding_test_bio/evaluation/mismatch_report_v2.txt'
with open(out, 'w', encoding='utf-8') as f:
    f.write(f'=== Heteronym=True validation ===\n\n')
    f.write(f'Total: {total}  Correct: {correct}  Mismatch: {total-correct}\n')
    f.write(f'Accuracy: {correct/total*100:.2f}%\n\n')

    if still_unmatched:
        f.write(f'Chars with no reading: {len(still_unmatched)}\n')
        f.write(f'  {still_unmatched}\n\n')

    if mismatches:
        f.write('STILL MISMATCHED (after considering all readings):\n\n')
        for (exp, all_cats), chars in sorted(mismatches.items()):
            f.write(f'{exp} => actual categories: {all_cats}\n')
            f.write(f'  {len(chars)} chars: {chars}\n\n')
    else:
        f.write('All characters have at least one reading matching expected category!\n')
